...
public class Dep_ScheduleController {
	...
	public String depschedulecalendar(@PathVariable String type, Model model,HttpSession session) {
		...
		if("my".equals(type)) {
			...
			for(Dept_ScheduleVO temp : dscode) {
				dataList.add(temp.sendDate());
			}
			
			model.addAttribute("dscode", dataList);
	...
	@ResponseBody
	public List<CalendarVO> depscheduleSetting(@PathVariable String type, Model model,HttpSession session){
		depschedulecalendar(type,model,session);
		return (List<CalendarVO>) model.asMap().get("dscode");
	}
	...
	public PagingVO<Dept_ScheduleVO> ajaxForlist(
			...
			depplanList(currentPage, searchVO, model,session);
	...
	public String depplanList(@RequestParam(name="page", required = false, defaultValue = "1") int currentPage,
		...
		detailSearch.setDept_code(empVO.getDept_code());
