...
public class My_ScheduleController {
	...
	public String myschedulecalendar(@PathVariable String type,Model model,HttpSession session) {
		...
		if("import".equals(type)) {
			List<My_ScheduleVO> mscode = dao.selectmyimport(detailSearch);
			List<CalendarVO> dataList = new ArrayList<CalendarVO>();
			for(My_ScheduleVO temp : mscode) {
				dataList.add(temp.sendDate());
			}
			
			model.addAttribute("mscode", dataList);
			model.addAttribute("type2","importantplan");
		}else {
			List<My_ScheduleVO> mscode = dao.selectmylist(empVO.getEmp_code());
	...
	@ResponseBody
	public List<CalendarVO> myscheduleSetting(@PathVariable String type,Model model,HttpSession session){
		myschedulecalendar(type,model,session);
		return (List<CalendarVO>) model.asMap().get("mscode");
	}
	...
	//내 일정 삭제(체크박스)
	@PostMapping("/plan/myplanbox/myplan/deletemy")
	public String deletedeptplan(@RequestParam String ms_code, Model model, HttpSession session) {
		
		if(ms_code.contains(",")) {
			String[] splited = ms_code.split(",");
			for(int i=0; i< splited.length; i++) {
				service.deleteMyschedule(splited[i]);
			}
		}else {
			service.deleteMyschedule(ms_code);          
		}
		return "MySchedule.plan";
		}
	...
	public PagingVO<My_ScheduleVO> ajaxFormylist(
			...
			MyImportSchedule(currentPage, searchVO, model,session);
			return (PagingVO<My_ScheduleVO>) model.asMap().get("pagingVO");
	}
