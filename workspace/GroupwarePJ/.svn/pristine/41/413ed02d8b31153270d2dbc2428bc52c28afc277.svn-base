              ...
              <form class="form-horizontal">
                <div class="card-body">
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                      <input type="text" class="form-control" id="ds_title" value="${deptschedule.ds_title}" name="ds_title" style="width: 400px;">
                    </div>
                  </div>
                  <div class="form-group row">
                    <label for="inputPassword3" class="col-sm-2 col-form-label">작성자</label>
                    <div class="col-sm-10">
                      <input type="text" class="form-control" id="writer" value="" name="emp_kor" readonly="readonly" style="width: 200px; background-color: white;">
                    </div>
                  </div>
...
<!-- 				<input type="text" class="emp_kor"  id="manager" name="" value="" style="height: 35px; width: 200px; border: 1px solid #ced4da; border-radius: .25rem;"> -->
<!-- 				주소록 버튼 -->
<!-- 				<button type="button" class="btn btn-secondary" id="btnAddEmp" data-toggle="modal" data-target="#modal-lg"> -->
<!-- 					<i class="fas fa-users"></i> -->
<!-- 				</button> -->
<!-- 			</div> -->
<%-- 			<input type="hidden" id="receiver_code" name="manager" value="${emp_code}"/> --%>

				<div class="form-group row">
				<label class="col-sm-2 col-form-label" id="emp_receiver">담당자</label> 
				<input type="text" class="emp_kor"  id="manager_name" name="" value="${deptschedule.manager_name }" style="height: 35px; width: 200px; border: 1px solid #ced4da; border-radius: .25rem;">
				<!-- 주소록 버튼 -->
				<button type="button" class="btn btn-secondary" id="btnAddEmp" data-toggle="modal" data-target="#modal-lg">
					<i class="fas fa-users"></i>
				</button>
			</div>
			<input type="hidden" id="receiver_code" name="manager" id="manager" value="${emp_code}"/>
...
#content {
   padding: 50px;
      ...
      <script>console.log(opener);
      	document.getElementById("ds_start").value = opener.startdate;  //시작일
		document.getElementById("ds_end").value = opener.enddate; 	   //종료일
		document.getElementById("ds_title").value = opener.title;	   //제목
		if(opener.emp_kor != ""){
			document.getElementById("writer").value = opener.emp_kor;	   //작성자
			
		}else{
			document.getElementById("writer").value = "${authUser.emp_kor}";
		}
		document.getElementById("ds_content").value = opener.content;  //내용		
		document.getElementById("manager").value = opener.manager;	   //담당자	
		document.getElementById("manager_name").value = opener.manager_name;	   //담당자	
		
		let ds_important = opener.important;
	
	$("select[name=ds_important]").val(ds_important);
...
<form id="searchForm" action="${cPath }/plan/depplanenroll">
...
<script>
	...
	$("#addAll").on("click", function() {
		...
		$("#receiver_code").val(added_emp_code); //수신자
		$("#emp_kors").val(added_emp); //수신자
		$("#manager_name").val(added_emp_name);
...
<script>
  ...
  $("#deptscheduleForm").on("submit",function(event){
			...
			$.ajax({
				...
				,success :function(resp){
					if(resp == 'ok'){
						alert("성공적으로 저장하셨습니다.")
//						location.href = "${cPath}/plan/depplancalendar/{type}";
