...
<!-- 					</select><br><br> -->
					...
					<c:if test="${empty worklog.wl_team }">
						<span class="text" style="margin-right: 30px">공유여부</span> <input type="checkbox" class="check" id="exampleCheck1" name="wl_share"<c:if test="${worklog.wl_share eq 'Y'}">checked</c:if>><br><br>
					</c:if>
...
<script>
	...
	$(".delBtn").on("click", function(){
		let wl_attcode = $(this).data("attno");
		worklogForm.append(
			$("<input>").attr({
				type : "text"
	...
	var deptTag = $("select[name='dept']").data("success", function(resp){
		...
		$.each(resp, function(i, dept){
			var selected = "";
			if(dept.dept_code == '${worklog.dept_code}'){
				selected = "selected"; 
			}
