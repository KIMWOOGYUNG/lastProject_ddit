...
<div id="content">
	...
	<a class="btn btn-primary btn-sm mx-2" href="${cPath }/note/writenote" style="float: right;">
    ...
    <a class="btn btn-primary btn-sm mx-2" id="moveBtn" href="#"  onclick="moveAction()" style="float: right;">
        <i class="fas fa-plus">
        </i>
    	 이동
    </a>
...
<table class="table table-hover projects" style="table-layout: fixed;">
  	<thead>
  		<tr > 
  		<th rowspan="1" colspan="1" id = "choice"><input type="checkbox" name="checkAll" id="th_checkAll" onclick="checkAll();"/></th>
  		...
  		success:function(resp){
  			...
  			if(msgList.length>0){
  			   ...
  			   $(".table-hover").on("click","tbody td",function(event){
  			    ...
  			    $(".table-hover").off();
  				trTags.push($("<tr>").append($("<td colspan='8'>").text("조건에 맞는 쪽지가 없습니다.")));
  				$("#pagingArea").empty();
  			}
