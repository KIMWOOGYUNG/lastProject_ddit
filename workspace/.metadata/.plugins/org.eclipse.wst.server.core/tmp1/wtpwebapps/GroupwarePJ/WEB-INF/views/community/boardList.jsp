					...
					<div class = "form-group" id = "searchUI">
						...
						<p class="input-group input-group-sm" style="width: 40%; float:left">
							...
							<div class="input-group-append">
								...
								onclick="location.href='<c:url value="/board/boardList/insertForm/${pos}"/>';" />
							</div>
						</p>
					</div>
				</div>
...
<form id="searchForm" action="${pageContext.request.contextPath }/board/boardList/${board_catag}">
			...
			if(boardList.length>0){
				$.each(boardList, function(idx, board){
					let trTag = $("<tr>").append(
									$("<td>").text(board.rnum)
									, $("<td>").html(
										$("<a>").attr("href", "${cPath }/board/boardList/boardView/" + board.board_code + "/${pos}")
