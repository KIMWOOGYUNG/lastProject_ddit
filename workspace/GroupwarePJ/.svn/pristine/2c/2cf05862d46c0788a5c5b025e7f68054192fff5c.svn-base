...
<form id="searchForm" action="${cPath }/work/teamwork/diary">
...
<script type="text/javascript">
	...
	$("#insertdiary").on("click", function(){
		 location.href = "${cPath}/work/mywork/insertdiary";
	});
			...
			if(worklogList.length > 0){
				$.each(worklogList, function(idx, worklog){
					...
					let trTag = $("<tr>").append(
									...
									, $("<td>").html($("<a>").attr("href", "${cPath }/work/teamwork/diaryView/" + worklog.wl_code).html(worklog.wl_title))
...
// 		data = data.substring(1);
		...
		if(confirm("일지를 삭제하시겠습니까 ? ")){
			$.ajax({
				url : '${cPath}/work/teamwork/trashbasket',
// 				type : 'delete',
				type : 'POST',
				dataType :'html',
