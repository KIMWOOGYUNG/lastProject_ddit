...
<div class = "content row" style="padding:30px 50px;">
	<div class="card card-primary col-sm-12" >
		...
		<div id = "top" style="width:80%;margin:0 auto; padding:0 10px;">
			...
			<button class="btn btn-info btn-sm float-sm-right" type="button" style="margin:5px;" onclick="receiveOK()">
			       보고승인
			</button>
		</div><br><br>
		...
		<div class="card card-primary" style="width: 77%; margin:0 auto;">
			...
			<div class="card-body" style="display: block;">
				<c:forEach items="${receive.report_attList }" var="attatch" varStatus="vs">
    ...
    <div id="pdfchange" style="width:100%;padding-bottom:100px;">
	    <table id="pdftable" border="0" cellpadding="0" cellspacing="0" id="sheet0" class="sheet0" style="width:80%;margin:0 auto;">
	        ...
	        <tbody>
	          ...
	          <tr class="row2">
	            <td class="column0">&nbsp;</td>
	            <td class="column19 style10 null style18" colspan="3" rowspan="3"><c:if test="${receive.report_sign ne null }"><img src="${cPath }/emp/file/${receive.report_sign }" style="height: 100px; width: 100px;"> </c:if></td>
	            <td class="column22 style10 null style21" colspan="3" rowspan="3"><c:if test="${receive.target_sign ne null and 'REP100' ne receive.report_stat}"><img src="${cPath }/emp/file/${receive.target_sign }" style="height: 100px; width: 100px;"> </c:if></td>
	          </tr>
	          <tr class="row3">
	            <td class="column0">&nbsp;</td>
	          </tr>
	          ...
	          <tr class="row5">
	            ...
	            <td class="column21 style9 null style31" colspan="4">${receive.emp_reporter}</td>
	          </tr>
	          <tr class="row6">
	            <td class="column0">&nbsp;</td>
	          ...
	          <tr class="row7">
	            ...
	            <td class="column4 style34 null style36" colspan="11" rowspan="2">${receive.team_code }</td>
	            <td class="column15 style37 s style23" colspan="2" rowspan="2">작성일자</td>
	            <td class="column17 style34 null style39" colspan="8" rowspan="2">${receive.report_date }</td>
	          </tr>
	          <tr class="row8">
	            <td class="column0">&nbsp;</td>
	          </tr>
	          ...
	          <tr class="row10">
	            ...
	            <td class="column4 style49 null style50" colspan="21">${receive.report_title }</td>
	          </tr>
	          <tr class="row11">
	            <td class="column0">&nbsp;</td>
	            <td class="column1 style40 s style53" colspan="3" rowspan="27">진행 업무    내용</td>
	            <td class="column4 style54 null style62" colspan="21" rowspan="27">${receive.report_content }</td>
	          </tr>
	          <tr class="row12">
	            <td class="column0">&nbsp;</td>
	          </tr>
...
<script>
	// 영구삭제
	function receiveOK(){	
		var report_code = "${receive.report_code}";
		
		if(confirm("보고를 승인 하시겠습니까?")){
			$.ajax({
				url : '${cPath}/work/workreport/receivereport/${report_code}',
				type : 'POST',
				dataType : 'json',
// 				data : {
// 					wl_code : report_code 
// 				},
				success : function(data){
					if(data.result == "FAIL"){
						...
						alert(data.message);
						location.href = "${cPath}" + data.goPage;
					}
				},
